...
public class UserRealm extends AuthorizingRealm{
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken arg0) throws AuthenticationException {
        ...
        UserEntity user = userBiz.findOneByAccount(token.getUsername());
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection arg0) {
        ...
        UserEntity dbUser = userBiz.findById(user.getId());

        if (dbUser.getAccount().equalsIgnoreCase("admin")) {
            info.addRole("admin");
            info.addStringPermission("admin");
        }
        if (dbUser.getAccount().contains("test")) {
            info.addRole("user");
        }

        if (StringUtils.isNoneBlank(dbUser.getPerms())) {
            List<String> perms = Arrays.asList(dbUser.getPerms().split(","));
            perms.stream().forEach(perm -> info.addStringPermission(perm));
        }
        return info;
    }
}
